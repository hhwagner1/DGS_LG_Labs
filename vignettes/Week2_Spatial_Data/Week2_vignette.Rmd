...
To use all the newest features, you will need the development version of `landscapemetrics v0.3`.
...
To check if a raster is suitable for `landscapemetrics` run the check_lanscape() function first. It checks if the units are in meters, the classes are coded as integer values and if the values are (most likely) discrete classes.
...
To calculate a single metric, just use the corresponding function. 
...
nlcd.landscape.aggr <- landscapemetrics::calculate_lsm(landscape = nlcd, 
                                                       level = "landscape", 
...
nlcd.subset <- landscapemetrics::calculate_lsm(landscape = nlcd, 
                                               what = c("lsm_c_pladj", 
                                                        "lsm_l_pr", 
                                                        "lsm_l_shdi"))
...
nlcd.subset.full <- landscapemetrics::calculate_lsm(nlcd, 
                                                    what = c("lsm_c_pladj", 
                                                             "lsm_l_pr", 
                                                             "lsm_l_shdi"), 
                                                    full_name = TRUE)
...
nlcd.subset.full.b <- dplyr::left_join(x = nlcd.subset,
...
cc.forest.a <- cc.nlcd[[4]]
cc.forest.b <- landscapemetrics::get_patches(nlcd, class = 42) # watch out: result is list with one entry
...
```{r}
  # show_patches(nlcd, class = "all", labels = FALSE)
  show_patches(landscape = nlcd, class = 42, labels = FALSE)
```

It is also possible to visualize only the core area of each patch using `show_cores()`. The core area are all cells that are further away from the edge of each patch than a specified edge depth (e.g. 5 cells). 

```{r}
  show_cores(landscape = nlcd, class = 42, edge_depth = 5,  labels = FALSE)
```
...
```{r}
show_lsm(landscape = nlcd, class = 42, what = "lsm_p_area", labels = FALSE)
``` 
