...
NOTE: THIS WILL BE TAKEN CARE OF: To get started, either set your working directory to where your data files are (Windows, RStudio), or change your terminal directory to launch R from inside the data files directory.

### a) Import data

```{r}
file <- scan(file = paste0(here::here(), "/data/dataNm2.txt"), what ="character", sep="\n", skip = 2)

#file <- scan(file = "dataNm2.txt", what ="character", sep="\n", skip = 2)
genotype <- NULL
for(locus in 1:100){
...
```{r}
genotype = read.table(file = paste0(here::here(), "/data/dataNm1.str"))[,-(1:2)]
#genotype = read.table(file = "dataNm1.str")[,-(1:2)]
...
```{r}
data <- read.table(paste0(here::here(), "/data/stickleback_data.txt"), 
                   sep="\t", as.is=T, check.names=F)
#data <- read.table("stickleback_data.txt",sep="\t",as.is=T,check.names=F)
```
...
We will use SNMF to estimate the number of genetic clusters (K) among our sampled populations.

NOTE: THIS TAKES A LONG TIME! Mayb just present the result?
...
We will use SNMF's cross-entropy criterion to infer the best estimate of K. The lower the cross-entropy, the better our model accounts for population structure. Sometimes cross-entropy continues to decline, so we might choose K where cross entropy first decreases the most.
...
```{r}
K=4
snmf = LEA::snmf(paste0(here::here(), "/data/stickleback.geno"), 
                 K = K, alpha = 100, project = "new")
#snmf = LEA::snmf("stickleback.geno", K = K, alpha = 100, project = "new")
```
...
### Visualize admixture proportions on a map

Import geographical coordinates for the populations: 

```{r}

sites <- read.csv(paste0(here::here(), "/data/stickleback_coordinates.csv"),
                  as.is=T, check.names=F, h=T)
#sites <- read.csv("stickleback_coordinates.csv", as.is=T, check.names=F, h=T)
```
...
Alternative with plotGoogleMaps:
Create map with pie charts depicting ancestry proportions:

```{r}
#coords <- data.frame(Longitude = coord.pop[,1], Latitude = coord.pop[,2])
#ggmap::qmplot(x=Longitude, y=Latitude, data = coords, 
#              source = "google", maptype = "terrain")

qpop.df <- data.frame(qpop)
names(qpop.df) <- c("A", "B", "C", "D")
data <- sp::SpatialPointsDataFrame(coord.pop, qpop.df)
sp::proj4string(data) <- sp::CRS("+init=epsg:4326")
pies <- plotGoogleMaps::pieSP(data, zcol = c(1:4), max.radius = 100)
pies$allele <- rep(names(qpop.df), nrow(qpop.df))
api <- "https://maps.googleapis.com/maps/api/js??sensor=false&v=3.18&key=AIzaSyCVz08TBp-MDGikOTlpZEHkx5yL_LnlEUk"
m <- plotGoogleMaps::plotGoogleMaps(pies, zcol = "allele")
```
