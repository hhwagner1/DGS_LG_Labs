...
source(paste0(here::here(), "/data/panel.cor.r"))
#source(system.file("extdata", "panel.cor.r", 
#                            package = "LandGenCourse"))
```

## 2. Estimate genetic values and variance components for an observed phenotypic trait
...
### e) Effect size: estimate variance components
...
Thus population and family each had a medium-size effect (> 9% variance explained) on d13c values.
...
### g) Describe and extract effects
...
We can extract the values with 'ranef'. Again, they are deviations from the global intercept (block 1). We will return to these values in the last part of the worked example to relate them to environmental site conditions.
...
The last thing we want to do in this lab is to test for correlations between environmental data and the family and population effects for the trait $δ^{13}C$ that we estimated above. Recall that $δ^{13}C$ is related to water use efficiency, hence we might expect it to be related to a number of bioclimatic variables.

Here, we'll focus on the correlation matrix. See 'Week 6 Bonus Material' for a multiple regression and variation partitioning to parital out the effects of 'Climate' vs. 'Geography'.

### a) Correlation of family effect with environment

```{r fig.height=7.5, fig.width=7.5}
index <- match(row.names(fam.eff), row.names(WWP.ecogen@E))
dat <- data.frame(fam.eff=fam.eff[,1], WWP.ecogen@E[index,])
...
- Which site variables show the strongest correlation with the family effect?
- Which site variables are significantly correlated with the family effect? 
- Overall, does the family effect seem to be related to the environment?

### b) Correlation of population effect with environment

We have 10 populations, each with 3 plots. Here we first extract 30 unique rows (one per plot) from the environmental data. Then we find for each plot the corresponding population and extract its population effect.
