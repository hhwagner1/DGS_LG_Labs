...
mod3 <- lme4::lmer(d13c ~ 1 + (1|population/family) + block, data = phen, REML = TRUE)
```

We are now ready to explore each of these models and to look at which model best fits our data. First, you can use the Anova() function in the car library to test the statistical significance of the fixed terms in each mod. This function let's us control the type of sums of squares (SS) being used, here type III sums of squares (see this eek's video, Part 2).
...
First we need to re-fit the models 'mod2' and 'mod3' with the option 'REML=FALSE' to obtain AIC values.

```{r}
mod2.noReml <- lme4::lmer(d13c~1+(1|family)+block,
                          data = phen, REML = FALSE)
mod3.noReml <- lme4::lmer(d13c ~ 1 + (1|population/family) + block,
                          data = phen, REML = FALSE)
```
...
```{R}
aic_vals <- c(AIC(mod1), AIC(mod2.noReml), AIC(mod3.noReml))
