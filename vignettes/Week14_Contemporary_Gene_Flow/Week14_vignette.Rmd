...
- how toconduct paternity analysis to quantify contemporary pollen flow, and 
...
**NOTE HW: The files are currently included as external data in LandGenCourse. Should we provide them as data sets (in R format, with meta data)?**
...
```{r, import}
dat <- gstudio::read_population(system.file("extdata",              "pulsatilla_genotypes.csv", package="LandGenCourse"), 
...
**NOTE HW: What are the coordinates? Same as for Dianthus? Are they Euclidean, like UTM?**

**NOTE HW: what are the genetic data? Microsats?**
...
**NOTE HW: Can  you explain what happens if mother and offspring both are heterozygous with the same two alleles?** Maybe point out examples in row 1?
...
Now the pollen pool data can be analysed as any other population genetic data. 
...
Here we use the `adonis` function from the `vegan` pacakage to do the analysis of variance on genetic distance matrices (AMOVA).

**NOTE HW: BTW, I got rather different results with 'adonis' than the other AMOVA implementations in Week 4.**
...
**NOTE HW: Could you do a hierarchical AMOVA with moms nested within patches?**

**NOTE HW: How about interpreting a phi statistic?** In the hierarchical analysis, it would be nice to test whether moms within the same patch sampled different pollen clouds, or quantify what percent of variation was among mothers within patches?
...
For example, we can test a basic hypothesis of isolation by distance by asking: are pollen pools from mothers that are geographically close more similar than those that are far? We test this by calculating pairwise genetic similarity (proportion of shared alleles) and relate this to geographic distance.

**NOTE HW: maybe make a note about pollen graphs that are similar to population graphs, which will eventually be included as Bonus material in Week 13?**

First, calculate matrix of pairwise genetic similarity among pollen pools:

```{r, dps}
dps <- gstudio::genetic_distance(pollen, stratum = "ID", mode = "Dps")
```
...
Note: 'dps' is a measure of genetic similarity, hence we expect it to decrease with geographic distance. We could plot (1 - dps) to get a plot of genetic distance against geographic distance.

```{r, plot}
par(mar=c(4,4,0,0))
plot(xy.dist[lower.tri(xy.dist)], dps[lower.tri(dps)], 
    xlab = "geographic distance (m)", 
     ylab = "proportion of shared alleles (dps)")
...
**NOTE HW: How about quantifying this with Mantel r to assess the strength of the relationship?** I get a Mantel r = -0.44, which is quite a bit! Hence I would omit the word 'slight' in the sentence above. 
...
**NOTE HW: Please explain output and interpretation.** What does Fij mean and how is it interpreted?

We see that not all fathers could be excluded for some of the offspring.

Now let's do the paternity for the entire data set. First, we define a function `get.parentage` that does the paternity analysis for the offspring of one mome (i.e., one family). 
...
We can use the `spider_plot` function from `gstudio` to visualize the paternity assignments. `spiderplot` takes the output from `paternity` and combines it with spatial XY coordinate information. The output includes the coordinates of both mother (X, Y) and father (Xend, Yend) for each offspring. Here we will also add in the population IDs so that we can visualize each of the seven populations separately.

**NOTE HW: does this assume that coordinates are in latlon format/projection?**
...
**NOTE HW: Any need to refit model with REML here?**

Let's plot the relationships:

```{r, plot_mom.isolation}
ggplot(pat.outcrossed, aes(x=log(mom.isolation), 
...
```{r, plot_flower.density}
ggplot(pat.outcrossed, aes(x=log(flower.density), 
       y=log(pollen.dist))) + geom_point() +
  stat_smooth(method="lm", se=F) + 
...
**NOTE HW: I suggest replacing 'tapply' by 'sapply'**
...
Specifically we have measured the proportion of forested landscape at radii of 50, 100, 250, 500, and 1000 metres from population centroids, and have measured population size as the total number of flowering plants per population.
...
Now we can run a model to see which variables best explain the proportion of immigrated pollen per population. Because we only have n=7 populations, we limit the models to a single explanatory variable. We first scale the variables to make sure estimates are comparable. 

Our response variable is a proportion, so we use a glm with binomial error distribution. This model does not include any random effects.
...
Here, we use the function `dredge` again to select the best model among those with only a single predictors. This is specified with the argument `m.lim=c(0,1)`.

```{r, scale.x}
MuMIn::dredge(mod2,m.lim=c(0,1) )
```

We see that the best model is the null intercept model, suggesting that none of the predictors fit the data very well. However, the next best model includes 'forest.250' and with a delta AICc of less than 2, we take this model to be equally likely. 

Interestingly, in our original paper with found that forest.500 was the best predictor, followed by population size. This suggests that the choice of paternity analysis really can make a difference.
...
**NOTE HW: There seems to be an outlier.** Which population is this? Do we have an explanation for it?

**NOTE HW: The labelling of the x-axis may be confusing here.** Maybe add 'standardized' to label? Is it necessary to standardize the variables here? I don't think you are interpreting the slope coefficient estimates or comparing them directly.

## 6. References

- DiLeo MF, R Holderegger and HH Wagner (*in press*). Contemporary pollen flow as a multiscale process: evidence from the insect-pollinated herb *Pulsatilla vulgaris*. Journal of Ecology.
