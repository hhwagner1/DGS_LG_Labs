...
No conceptual review for this lab, as no data analysis was performed.

### b) R programming exercise

**Task:** Import the data set `pulsatilla_genotypes`, which we'll use in a later lab, into 'gstudio'.

**Hints:**

- The file `pulsatilla_genotypes.csv` is a available as a system file in the `extdata` folder of the `LandGenCourse` package. You you can access it with `system.file("extdata", "pulsatilla_genotypes.csv", package = "LandGenCourse")`. The file has a header row.
- Adapt the code from section 2.c to import the raw data set. View it. How are the genetic data coded? 
- Make sure the 'gstudio' package is loaded.
- Adapt the code from section 5 to import the genetic data with 'gstudio'. The loci are in columns 6 - 19. What setting for 'type' is appropriate for this data set? Check the help file for 'read_population'. 
- Use the function `str` to check the imported data. Does each variable have the correct data type? Note: there should be 7 variables of type 'locus'.
