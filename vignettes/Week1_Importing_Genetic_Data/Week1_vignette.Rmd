...
**Task:** Import the data set `pulsatilla_genotypes`, which we'll use in a later lab, into `gstudio` and convert it to a genind objec 

**Hints:**

a) **Load packages**: Make sure the `gstudio` package is loaded.
b) **View system file**: The file `pulsatilla_genotypes.csv` is a available as a system file in the `extdata` folder of the `LandGenCourse` package. You you can access it with `system.file("extdata", "pulsatilla_genotypes.csv", package = "LandGenCourse")`. The file has a header row. Adapt the code from section 2.c to import the raw data set. View it. How are the genetic data coded? 
...
d) **Check imported data**: Use the function `str` to check the imported data. Does each variable have the correct data type? Note: there should be 7 variables of type 'locus'.
e) **Make sense of variables**: Create a bulleted list with three entries, one for each group of variables in this data set: identifiers, spatial coordinates, genetic data. For each group, list the variable names and their data type (e.g., numeric, integer, character, logical, factor, locus). Check the cheatsheet "R markdown language" if needed.
f) **Convert to genind object**: Modify the code from section 5.e to convert the data to from gstudio to a genind object. Ignore the warning about duplicate labels. Print a summary of the genind object. What is the range of the number of genotyped individuals per population in this dataset?
