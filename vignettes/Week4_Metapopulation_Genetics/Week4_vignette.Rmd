...
- What is the biological interpretation of a positive residual for site-level FST? What processes might lead to this?
- And for a negative residual?
...
Note: here we specify 'zoom = 12', whereas the default 'zoom = "auto" would set it at 11 for this example. For a 'toner-lite' map from 'stamen', the zoom level affects the resolution and level of detail of the backgroud map, not the spatial extent. For a 'google' map, among others, zoom affects level of detail and the extent. Don't ask us why...  
...
myMap <- ggmap::qmplot(Longitude, Latitude,  data = as.data.frame(dd.spatial),
              source = "stamen", maptype = "toner-lite", zoom = 12,  
...
The code below exports the last plot as a 'png' (Portable Network Graphics) file, specifying canvas size and resolution. To run the code, un-comment the lines by removing '#' at the beginning of each line.

```{r}
#require(here)
#if(!dir.exists(paste0(here(),"/output"))) 
#      {dir.create(paste0(here(),"/output"))}
#ggplot2::ggsave("ResidualMap.png", width = 7, height = 5.5, 
#                  units = "in", dpi = 300)
```
...
```{r fig.height=7, fig.width=8}
par(mfrow=c(2,2))
plot(mod.diff.minus2, 
     labels.id = dd.spatial@data$SITE[a==FALSE])
par(mfrow=c(1,1))
```
