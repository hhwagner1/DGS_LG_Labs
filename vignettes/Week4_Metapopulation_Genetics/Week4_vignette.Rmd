...
a) **Load packages**: You may want to load the packages `gstudio`, `dplyr` and `ggplot2`. Alternatively, you can use `::` to call functions from packages.
b) **Recreate site-level dataset with "lonlat" coordinates**. Rerun your Week 2 code. This should produce a SpatialPointsDataFrame object `Pulsatilla.longlat` with coordinates in "lonlat" projection. Use the function `as.data.frame` to print the dataset including the spatial coordinates. Check the variable names. 
c) **Plot sites onto map from internet**: adapt the following code from section 3.c to download a stamen map from the internet and plot the sites on it: 
`myMap <- ggmap::qmplot(Longitude, Latitude,  data = as.data.frame(dd.spatial), source = "stamen", maptype = "toner-lite")`
Display the map `myMap`. 


b) **Recreate dataset**: Re-use your code from Week 3 exercise to import the dataset `pulsatilla_genotypes.csv` into `gstudio`.
c) **Drop offspring**: Use `dplyr::filter` to drop all offspring, retaining only adults (`OffID == 0`). 
d) **Convert to ecogen object**: Use the function `EcoGenetics::gstudio2ecogen` to convert to an `ecogen` object. Here is a generic code example from the help file:
`toeco <- gstudio2ecogen(togstudio, lat = "Latitude", lon = "Longitude", ID = "ID", struct = "pop")`. Adapt the code to the Pulsatilla dataset. Note that `lat` refers to the y coordinate and `lon` to the x coordinate. The actual coordinates in this dataset are not in "lonlat" format. Print the object to check that the variables have been correctly added to the slots XY, G and S.
e) **Convert to ecopop object**: Use the function `EcoGenetics::ecogen2ecopop` to aggregate the individual data to population-level data. Here is a generic code example from the help file: `my_ecopop <- ecogen2ecopop(eco, hier = "pop")`.
f) **Add site-level attribute data**: 
...
### e) Convert to ecopop object

```{r}
Adults.ecopop <- ecogen2ecopop(Adults.ecogen, hier = "Population")
```

### e)

```{r}
read.csv(system.file("extdata", "pulsatilla_population.csv", 
                     package="LandGenCourse"), header=TRUE)

read.csv(system.file("extdata", "pulsatilla_momVariables.csv", 
                     package="LandGenCourse"), header=TRUE)
```
