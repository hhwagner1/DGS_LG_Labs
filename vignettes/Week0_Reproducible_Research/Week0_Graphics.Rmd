...
With basic R graphics, we specify these layers as arguments passed to the plotting function (`plot()`, `boxplot()`, etc.). The ggplot2 library takes a different approach, allowing you to specify these components separately and literally add them together like components of a linear model. We'll see below what that means.
...
If the data on the x-axis is not discrete but measured on a single, numeric variable, then the `hist()` function can take the data and categorize it into groups (bins) based upon the density of the underlying data.
