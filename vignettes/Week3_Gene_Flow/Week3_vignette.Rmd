...
### b) Data set (same as used in Weeks 1 & 2)
...
```{r, fig.show='hold'}
  ...
  barplot(Sum$Hexp, ylim=c(0,1), ylab="Expected heterozygosity", las=3)
  barplot(Sum$Hobs, ylim=c(0,1), ylab="Observed heterozygosity", las=3)
``` 
...
GD.ind.kosman <- PopGenReport::gd.kosman(Frogs.genind)
GD.ind.smouse <- PopGenReport::gd.smouse(Frogs.genind, verbose=FALSE)  # GenAlEx
GD.ind.propShared <- adegenet::propShared(Frogs.genind)       # 1 - GD.ind.kosman
```
...
### d) Additional genetic distances (gstudio)

```{r}
# Import data for analysis with gstudio (see Week 1)
...
GD.ind.amova <- gstudio::dist_amova(Frogs.gstudio)
{
# Population-level genetic distances
GD.pop.Bray <- gstudio::dist_bray(Frogs.gstudio, stratum="Pop")
GD.pop.Cavalli <- gstudio::dist_cavalli(Frogs.gstudio, stratum="Pop")  # same as GD.pop.Edwards?
GD.pop.cGD <- gstudio::dist_cgd(Frogs.gstudio, stratum="Pop")
GD.pop.Eucl <- gstudio::dist_euclidean(Frogs.gstudio, stratum="Pop")
GD.pop.Jaccard <- gstudio::dist_jaccard(Frogs.gstudio, stratum="Pop")
#GD.pop.Nei <- gstudio::dist_nei(Frogs.gstudio, stratum="Pop")  # Same as above (?)
#GD.pop.ss <- gstudio::dist_ss(Frogs.gstudio, stratum="Pop")  # Does not work
}
```
...
```{r}
GD.ind <- list(amova.gstudio = GD.ind.amova,
               kosman.PopGenReport = GD.ind.kosman,
               smouse.PopGenReport = GD.ind.smouse,
...
GD.pop <- list(pairwiseFst.hierfstat = Pairwise.fst,
               ...
               Bray.gstudio = GD.pop.Bray,
               Cavalli.gstudio = GD.pop.Cavalli,
               cGD.gstudio = GD.pop.cGD,
               Eucl.gstudio = GD.pop.Eucl,
               Jaccard.gstudio = GD.pop.Jaccard,
...
There are multiple implementations of Analysis of Molecular Variance (AMOVA) in R. The package 'poppr' provides an interface to calculate AMOVA from a 'genind' object using either the implementation in 'ade4' or in 'pegas'. See also: 
