...
g) **Convert to genind object**: adapt your code from Week 1 exercise to convert the dataset with all adults, `Pulsatilla.adults`, to a genind object. Plot the object to check that the data have been correctly imported. Is the number of rows equal to the number of adults that you found above? 
h) **Check polymorphism**: Plot a summary of the `genind` object to check whether markers are polymorphic: what is the range of expected heterozygosity among the loci?
...
k) **Calculate Hexp by site**: adapt code from section 3.b to calculate `Hexp` by site and locus, then take the mean across all loci (`Hexp.pop`). 
