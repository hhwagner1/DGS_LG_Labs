...
### c) Assemble (and save) distance matrices
...
### d) Genetic distances among individuals 

This part is commented out with '#' as it takes longer to calculate. 

```{r}
#GD.ind.smouse <- PopGenReport::gd.smouse(Snails.genind, verbose=FALSE) 
...
Assemble individual-level distance matrices:
...
### e) Export genetic distance matrices
...
First, we calculate geographic (Euclidean) distances 'Dgeo' with the 'dist' function.
...
There seems to be a general pattern across distance matrices, with a slight increas of genetic distance over shorter distances and then som decrease over large distances. The odd one out is again conditional genetic distance cGD (for good reason: see Week 13).
...
corm.lin <- EcoGenetics::eco.cormantel(M = D, XY = dd.spatial@coords,  
            nsim = 99, latlon=TRUE, alternative="less")

corm.rank <- EcoGenetics::eco.cormantel(M = GD.pop$propShared.PopGenReport, 
             XY = dd.spatial@coords,  nsim = 199, latlon=TRUE,
...
nb.del <- adegenet::chooseCN(xy = dd.spatial@data[,1:2], result.type = "nb", plot.nb = FALSE, type = 1)
nb.gab <- adegenet::chooseCN(xy = dd.spatial@data[,1:2], result.type = "nb", plot.nb = FALSE, type = 2)
nb.rel <- adegenet::chooseCN(xy = dd.spatial@data[,1:2], result.type = "nb", plot.nb = FALSE, type = 3)
nb.mst <- adegenet::chooseCN(xy = dd.spatial@data[,1:2], result.type = "nb", plot.nb = FALSE, type = 4)
nb.nbd <- adegenet::chooseCN(xy = dd.spatial@data[,1:2], result.type = "nb", plot.nb = FALSE, type = 5, d1=100, d2=15000)
nb.4nn <- adegenet::chooseCN(xy = dd.spatial@data[,1:2], result.type = "nb", plot.nb = FALSE, type = 6, k = 4)
...
plot(nb.nbd, coords=dd.spatial@data[,1:2]); title(main = "Neighbor distancer")
