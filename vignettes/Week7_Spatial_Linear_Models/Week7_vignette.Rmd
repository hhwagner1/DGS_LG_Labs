...
- **Dianthus**: Spatial point data frame with population-level data (patch characteristics, grazing regime, genetic diversity, 15 alternative connectivity indices Si), included in package 'LandGenCourse'. To load it, type: data(Dianthus)
...
## 2. Test regression residuals for spatial autocorrelation
...
```{r}
load(paste0(here::here(), "/vignettes/Week7_Spatial_Linear_models/Dianthus.Rdata"))
#data(Dianthus)
```
...
```{r}
round(matrix(cor(Dianthus@data$A, Dianthus@data[,18:32], 
...
### c) Test for spatial autocorrelation (Moran's I):
...
## 3. Fit models with spatially correlated error (GLS) with package 'nlme'
...
## 4. Fit spatial simultaneous autoregressive error models (SAR) with package 'spdep'
...
## 5. Spatial filtering with MEM using package 'spmoran'
...
## 6. Fit spatially varying coefficients models with package 'spmoran'
...
Keep in mind that 'IBR' and 'PatchSize' showed a strong correlation. The parameter estimates could therefore depend quite a bit on the other variables. To help with the interpretation, let's repeat the last analysis just with 'IBD', without 'PatchSize'. 
...
- The evidence is not conclusive, however, the observed patterns could also be explained by population size, which in this species seems to be associated with population size. This makes sense if smaller patches contain smaller populations with higher rates of genetic drift.
