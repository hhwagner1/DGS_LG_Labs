...
Dianthus <- data.frame(Dianthus[,c("patch", "samples", "pop09", "System", "intensity", "Ha", "Elements2008.4",  "Dc.89", "Dc.09", "F", "Ne", "I", "Ho", "He", "A", "PA", "FIS")], Dianthus[,15:29], Dianthus[,c("x", "y")])
# Fix error?
Dianthus[Dianthus$patch=="G21a", "pop09"] <- 2
require(sp)
sp::coordinates(Dianthus) <- ~ x +  y
sp::proj4string(Dianthus)=CRS("+proj=tmerc +lat_0=0 +lon_0=9 +k=1 +x_0=3500000 +y_0=0 +ellps=bessel +datum=potsdam +units=m +no_defs") 
#proj4string(Dianthus)=CRS("+proj=utm +zone=32 +ellps=WGS84 +datum=WGS84 +units=m #+no_defs") 
 
save(Dianthus, file = paste0(here::here(),                           "/vignettes/Week7_Spatial_Linear_Models/Dianthus.Rdata"))
