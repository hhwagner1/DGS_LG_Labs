...
The dataset 'DD.ecogen' is included in the package 'LandGenCourse' (to load it, type: data(SS.ecogen). It contains individual-level data in the following slots:

- XY: Spatial coordinates (FORMAT?) (population-level)
- G:  Microsatellite loci (columns = loci, rows = individual)
- A: Table of allele frequencies (columns = alleles, rows = individual)
- E: Ecological site variables
- S: Hierarchical structure variables (site, cluster,  year)
- C: Calculated genetic variables (site-level Fst, etc.)
...
Genetic data (spatial: one year per site, 25 sites):

```{r}
## Import individual-level data:
#Snail.genind <- import2genind("./vignettes/Week5_Habitat_Connectivity/Genetic_LCBDs/Data_25pops.gen", ncode=3)
#Snail.ecogen <- genind2ecogen(Snail.genind)
#Snail.ecogen@S <- data.frame(pop=Snail.genind@pop)
##Snail.gstudio <- ecogen2gstudio(Snail.ecogen, type="codominant")

# Aggregate at population level:
#Snail.genpop <- genind2genpop(Snail.genind)
#Snail.genpop@other$Population <- row.names(Snail.genpop@tab)
#Snail.genpop@other$Env <- read.table('./vignettes/Week5_Habitat_Connectivity/Genetic_LCBDs/env_25pops.txt', header=TRUE, row.names=1)
#Snail.genpop@other$Table4 <- read.csv('./vignettes/Week5_Habitat_Connectivity/Lamy/Lamy_etal_MolEco2012_Table4.csv', header=TRUE)

#cbind(Snail.genpop@other$Population, row.names(Snail.genpop@other$Env), 
#      as.character(Snail.genpop@other$Table4$SiteID))
```

Full genetic data (spatial and temporal, 42 combinations of site and year):

```{r}

Snail.all.gstudio <- gstudio::read_population(
  "./vignettes/Week5_Habitat_Connectivity/Lamy/MEC_5478_sm_Data.csv", 
...
Snail.all.ecogen <- EcoGenetics::gstudio2ecogen(Snail.all.gstudio, ID = "INDIVIDUAL", 
                                                ...
                                                struct =c("POP_code", "SITE", "YEAR"))

Snail.all.genind<- ecogen2genind(Snail.all.ecogen)
Snail.all.genind@pop <- Snail.all.genind@strata$POP_code

# Import tables
Table2 <- read.csv('./vignettes/Week5_Habitat_Connectivity/Lamy/Lamy_etal_MolEco2012_Table2.csv', header=TRUE)
Table4 <- read.csv('./vignettes/Week5_Habitat_Connectivity/Lamy/Lamy_etal_MolEco2012_Table4.csv', header=TRUE)
XY <- data.frame(Latitude = sapply(split(Snail.all.gstudio$North.WGS84, 
                                         Snail.all.gstudio$POP_code), unique),
                 Longitude = sapply(split(Snail.all.gstudio$West.WGS84, 
                                          Snail.all.gstudio$POP_code), unique))
XY$SiteID <- row.names(XY)

# Aggregate at population level:
Snail.all.genpop <- genind2genpop(Snail.all.genind, pop = Snail.all.genind@strata$POP_code)

tmp <- dplyr::left_join(data.frame(SiteID=row.names(Snail.all.genpop@tab)), Table2)
tmp <- dplyr::left_join(tmp, Table4[,c("POP_code", "Type", "FST.GESTE", "Size", 
                        "V", "C", "Stab", "D", "APA", "NLT", "Fst.temp")], 
                        by=c("SiteID" = "POP_code"))
tmp <- dplyr::left_join(tmp, XY)
Snail.all.genpop@other$Table2 <- tmp
```
