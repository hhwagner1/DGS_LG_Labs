...
##  *  Genetic data on the freshwater snail D. depressissimum                 ##
##  *  from the metacommunity in Guadeloupe (Grande-Terre)                    ##                                 
################################################################################
## Thomas LAMY modified on November 17th 2017
## R 3.3.3
...
setwd()
## load R function genetic.LCBD()        
source("genetic.LCBD.R")
## load R function is.metric()        
source("is.metric.R")

## load genetic data on D. depressissimum (749 individuals; 25 populations; 10 microsatellite markers)
data_ind <- import2genind("Data_25pops.gen", ncode=3)
...
res.LCBD <- genetic.LCBD(data.ind, D.opt=2, perm.opt=3, nperm=999) # 1 min
...
## Pev, Des, Hen, Ptc
## Porte Enfer Vigie, Desbonnes, L'Henriette, Pointe des Châteaux

## Plot the LCBD values on maps of the Guadeloupe snail sites
require(SoDA)
env_25pop <- read.table(file.choose(), header=TRUE, row.names=1)
